...
class DatasetLoader:
    def __init__(self, dataset_config: DatasetConfig):
        ...
        self.dataset_split = dataset_config.dataset_split
        self.cache_dir = dataset_config.dataset_cache_dir
        self.dataset_limit = dataset_config.dataset_limit

    def load_dataset(self) -> Dict[Path, str]:
        logger.info(
            f"Загрузка датасета {self.dataset_path}/{self.dataset_name}, разделение: {self.dataset_split}"
        )
        ...
        mapped = dataset.map(
            extract_audio_and_sentence,
            batched=True,
            remove_columns=dataset.column_names,
        )
        filtered = [
            (Path(p), s)
            for p, s in zip(mapped["audio_paths"], mapped["sentences"])
            if s and any(c.isalpha() for c in s)
        ]
        audio_paths_with_transcriptions = dict(filtered)

        logger.info(
            f"Загружено {len(audio_paths_with_transcriptions)} примеров из датасета"
        )
